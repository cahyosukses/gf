				...
				if (isset($Array->field) && isset($Array->value)) {
					if ($Array->type == 'numeric') {
						if ($Array->comparison == 'eq') {
							...
							$StringFilter .= "AND " . $Field." > '".$Array->value."' ";
						} else if ($Array->comparison == 'eq-lt') {
							$StringFilter .= "AND " . $Field." <= '".$Array->value."' ";
						} else if ($Array->comparison == 'eq-gt') {
							$StringFilter .= "AND " . $Field." >= '".$Array->value."' ";
						}
					} else if ($Array->type == 'date') {
						if ($Array->comparison == 'eq') {
							$StringFilter .= "AND DATE(" . $Field . ") = '".ConvertDateToQuery($Array->value)."' ";
						} else if ($Array->comparison == 'lt') {
...
if (! function_exists('GetStringFromList')) {
	function GetStringFromList($Param) {
		$ArrayFieldYesNo = array(
			'supplier_active', 'agent_active', 'car_active', 'customer_active', 'driver_active',
			'guide_active', 'tour_code_active', 'tour_pattern_active', 'employee_allowlogin', 'employee_active'
		);
		$ArrayEmptyOrExist = array('driver_photo', 'employee_avatar');
...
if (! function_exists('SetUpKurs')) {
	function SetUpKurs() {
		$SelectQuery = "SELECT * FROM ".OTHER." WHERE content_name IN ('Kurs Konversi Dolar', 'Kurs Konversi Yen')";
		$SelectResult = mysql_query($SelectQuery) or die(mysql_error());
		while (false !== $Row = mysql_fetch_assoc($SelectResult)) {
			if ($Row['content_name'] == 'Kurs Konversi Dolar') {
				define('KURS_DOLAR', $Row['content']);
			} else if ($Row['content_name'] == 'Kurs Konversi Yen') {
				define('KURS_YEN', $Row['content']);
			}
		}
	}
}

if (! function_exists('CoversionPrice')) {
	function CoversionPrice($Param) {
		if (! defined('KURS_DOLAR')) {
			SetUpKurs();
		}
		
		$Value = 0;
		if ($Param['currency_id'] == 1) {
			$Value = $Param['value'];
		} else if ($Param['currency_id'] == 2) {
			$Value = KURS_YEN * $Param['value'];
		} else if ($Param['currency_id'] == 3) {
			$Value = KURS_DOLAR * $Param['value'];
		}
...
if (! function_exists('ShowLineBreak')) {
	function ShowLineBreak($String) {
		return str_replace("\n", "<br />", $String);
	}
}

if (! function_exists('ShowGfTime')) {
	function ShowGfTime($Time) {
		if (empty($Time)) {
			return '<span class="red">-</span>';
		} else if ($Time == '0000-00-00 00:00:00') {
			return '<span class="red">-</span>';
		}
		
		$CurrentUnixTime = ConvertToUnixTime(date('Y-m-d H:i:s'));
		$DataUnitTime = ConvertToUnixTime($Time);
		$DiffTime = $DataUnitTime - $CurrentUnixTime;
		
		$StringTime = '';
		$ClassText = ($DiffTime < 0) ? 'red' : '';
		
		$Day = floor($DiffTime / (24 * 60 * 60));
		$Day = ($Day <= 0) ? 0 : $Day;
		$StringTime .= ($Day > 0) ? $Day . ' Hari ' : '';
		
		$Hour = floor($DiffTime / (60 * 60));
		$Hour = $Hour % 24;
		$Hour = ($Hour <= 0) ? 0 : $Hour;
		$StringTime .= ($Hour > 0) ? $Hour . ' Jam ' : '';
		
		$Minute = floor($DiffTime / 60);
		$Minute = $Minute % 60;
		$StringTime .= ($Minute > 0) ? $Minute . ' Menit ' : '';
		
		$StringTime .= ($DiffTime < 60 && $DiffTime > 0) ? $DiffTime . ' Detik' : '';
		
		if (empty($StringTime)) {
			$StringTime = 'sekarang';
			$ClassText = 'red';
		}
		$StringTime = '<span class="' . $ClassText . '">' . trim($StringTime) . '</span>';
